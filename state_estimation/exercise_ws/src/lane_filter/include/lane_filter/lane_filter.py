...
class LaneFilterHistogramKF():
    ...
    def predict(self, dt, left_encoder_delta, right_encoder_delta):
        ...
        # compute dD and dphi
        dD_left = 2*np.pi*self.wheel_radius*left_encoder_delta/self.total_ticks
        dD_right = 2*np.pi*self.wheel_radius*right_encoder_delta/self.total_ticks

        dD_k = (dD_left+dD_right) / 2.
        dphi_k = (dD_right-dD_left) / self.wheel_distance

        eD = dD_k/2.*0.1 if not self.wheel_trim else dD_k/2.*self.wheel_trim
        ephi = np.sin(eD/dD_k) if abs(dD_k) > EPS else 0
        Q = self.fQ(eD, ephi)
    ...
    def update(self, segments):
        ...
        '''
        d_grid, phi_grid = np.mgrid[self.d_min:self.d_max:self.delta_d,
                                   self.phi_min:self.phi_max:self.delta_phi]
        grid_value = np.array([[d, phi] for d, phi in zip(np.ravel(d_grid), np.ravel(phi_grid))])
        d_grid_idx, phi_grid_idx = np.mgrid[0:d_grid.shape[0], 0:d_grid.shape[1]]
        grid_idx = np.array([[d_i, phi_i] for d_i, phi_i in zip(np.ravel(d_grid_idx), np.ravel(phi_grid_idx))])
        self.z = np.array([0., 0.])
        for idx, value in zip(grid_idx, grid_value):
            likelihood = measurement_likelihood[idx[0], idx[1]] if (not measurement_likelihood is None \
                                                                and measurement_likelihood[idx[0], idx[1]]) \
                                                                else 0
            print(likelihood, value)
            self.z += likelihood * value
        '''
